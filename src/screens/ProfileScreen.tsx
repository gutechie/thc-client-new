...
export const ProfileScreen = () => {
    ...
    const {data, isLoading: isUserLoading, isError: isUserError, error: userError} = useGetCurrentUserQuery();
    ...
    const saveUpdatedDetails = async () => {
        ...
        try {
            ...
            Toast.show({title: 'Success', description: 'Your profile has been updated.'})
        } catch (e) {
            console.log(e)
            Toast.show({title: 'Error updating profile details', description: e.message})
        }
    }
    ...
    return (
        <KeyboardAvoidingView>
            <Box>
                ...
                <Box position={"absolute"} bottom={0} w={"full"} px={4} bg={"white"} py={4}>
                    <Button colorScheme={"orange"} onPress={saveUpdatedDetails}>Save</Button>
                </Box>
            </Box>
        </KeyboardAvoidingView>
    );
