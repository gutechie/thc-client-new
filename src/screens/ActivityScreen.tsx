...
export const ActivityScreen = () => {
    ...
    const {data, isLoading, isError, error} = useListUserActivitiesQuery(queryParams.toString())
    ...
    if (isError) {
        return <Box><Text>{error.error}</Text></Box>
    }

    function findMeasuringMetric(item) {
        return item.metrics.find((metric) => metric.name === item.activity.measuring_metric);
    }

    return <Box p={4}>
        <FlatList data={data.data} renderItem={({item}) => <Box my={"2"} p={4} bg={"white"} rounded={"md"} shadow={"6"}>
                <Heading fontSize={"sm"}>
                    {sentenceCase(item.activity.name)}
                </Heading>
                <Text color={"muted.500"}>{format(new Date(item.activity_date), 'PPP p')}</Text>
            <HStack justifyContent={"space-between"} mt={2}>
                <HStack space={2}>
                    <Text>{sentenceCase(item.activity.measuring_metric)}</Text>
                    <Text>
                        {findMeasuringMetric(item)?.stats} {findMeasuringMetric(item)?.unit}
                    </Text>
                </HStack>
                <HStack space={2}>
                    <Text>Tokens</Text>
