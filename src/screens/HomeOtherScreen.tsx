...
export const HomeOtherScreen = () => {
  ...
  return (
    <Box px={4} mb={48}>
      <Heading size={"md"} textAlign={"center"} my={4} color={"orange.500"}>
      ...
      />
      {criteriaData && metrics && userWithProfile && (
        <Box mb={4}>
          <Select
            selectedValue={selectedCriterion}
            w={"full"}
            accessibilityLabel="Choose Criteria"
        ...
        (criterionAvailable ? (
          <ScrollView>
            {metrics.data.map((metric) => (
              <PerformanceChartSingle
                dateRange={makeDateRanges()[selectedDate]}
                metric={metric}
                key={metric.id.toString()}
                criterion={criteriaData.data.find(
          ...
          <Text my={4}>
            You have not provided the value for{" "}
            {findSelectedCriterion(selectedCriterion).label}. Please edit your
            profile details to update the value or select other criteria
          </Text>
        ))}
    </Box>
  );
